...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("countChange should work for multi-coins") {
    ...
    check(10, List(5, 10), 2)
    check(50, List(1, 2, 5, 10), 341)
    check(250, List(1, 2, 5, 10, 20, 50), 177863)
  }


  test("parCountChange should work for multi-coins (money threshold)") {
    def check(money: Int, coins: List[Int], expected: Int) =
  ...
  test("parCountChange should work for multi-coins (coins threshold)") {
    def check(money: Int, coins: List[Int], expected: Int) =
  ...
  test("parCountChange should work for multi-coins (combined threshold)") {
    def check(money: Int, coins: List[Int], expected: Int) =
