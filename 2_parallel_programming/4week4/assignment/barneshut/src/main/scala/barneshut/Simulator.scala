...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    val updatedBoundaries = new Boundaries()

    updatedBoundaries.minX = boundaries.minX
    updatedBoundaries.maxX = boundaries.maxX
    updatedBoundaries.minY = boundaries.minY
    updatedBoundaries.maxY = boundaries.maxY

    if (body.x < updatedBoundaries.minX) updatedBoundaries.minX = body.x
    if (body.x > updatedBoundaries.maxX) updatedBoundaries.maxX = body.x
    if (body.y < updatedBoundaries.minY) updatedBoundaries.minY = body.y
    if (body.y > updatedBoundaries.maxY) updatedBoundaries.maxY = body.y
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    ...
    mergedBoundaries.minX = Math.min(a.minX, b.minX)
    mergedBoundaries.maxX = Math.max(a.maxX, b.maxX)

    mergedBoundaries.minY = Math.min(a.minY, b.minY)
    mergedBoundaries.maxY = Math.max(a.maxY, b.maxY)
  ...
  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    ...
    parBodies.aggregate(new SectorMatrix(new Boundaries, SECTOR_PRECISION))(_ += _, _ combine _)
  }
  ...
  def updateBodies(bodies: Seq[Body], quad: Quad): Seq[Body] = timeStats.timed("update") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    val p: ParSeq[Body] = parBodies.map(_.updated(quad))
    p.seq
  }
