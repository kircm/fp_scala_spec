...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if (points.size > 0) points.groupBy(point => findClosest(point, means))
    else means.map(mean => (mean, GenSeq[Point]())).toMap
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    val filteredMeans = oldMeans.zip(newMeans).filter(pair => pair._1.squareDistance(pair._2) > eta)
    filteredMeans.isEmpty
  }
