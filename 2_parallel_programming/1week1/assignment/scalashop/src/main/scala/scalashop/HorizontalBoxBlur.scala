...
object HorizontalBoxBlur {
  ...
  def blur(src: Img, dst: Img, from: Int, end: Int, radius: Int, numTask: Int): Unit = {
    val lastPixel = end - 1
    println(s"-> HorizontalBoxBlur.blur() task number: $numTask - Pixel rows from $from to $lastPixel")
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = {
    /* Using zip */
    val totalPixelRows: Int = src.height
    val pixelsPerSection: Int = totalPixelRows / numTasks
    println(s"Num tasks: $numTasks")
    println(s"Total pixel rows: $totalPixelRows")
    println(s"Pixels per section: $pixelsPerSection")

    (1 to numTasks)
      .zip(0 until totalPixelRows by pixelsPerSection)
      .map { case (numTask, currentPixelFrom) =>
        if (numTask < numTasks)
          task(blur(src, dst, currentPixelFrom, currentPixelFrom + pixelsPerSection, radius, numTask))
        else
          task(blur(src, dst, currentPixelFrom, src.height, radius, numTask))
      }
      .map(_.join)
  }
