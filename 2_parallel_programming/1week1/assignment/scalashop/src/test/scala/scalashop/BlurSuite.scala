...
class BlurSuite extends FunSuite {
  ...
  test("boxBlurKernel should return the correct value on an interior pixel " +
    ...
    val result = boxBlurKernel(src, 1, 2, 1)
    assert(result === 12,
      s"(boxBlurKernel(1, 2, 1) should be 12, " +
        s"but it's ${result})")
  }

  test("boxBlurKernel should work for a bounding pixel " +
    "of a 3x4 image with radius 1") {
    val src = new Img(3, 4)
    src(0, 0) = 0; src(1, 0) = 1; src(2, 0) = 2
    src(0, 1) = 3; src(1, 1) = 4; src(2, 1) = 5
    ...
    val result = boxBlurKernel(src, 2, 1, 1)
    assert(result === 4,
      s"(boxBlurKernel(2, 1, 1) should be 4, " +
        s"but it's ${result})")
  }
